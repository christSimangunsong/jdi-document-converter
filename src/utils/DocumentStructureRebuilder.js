class DocumentStructureRebuilder {
  ...
  detect(line) {
    ...
    if (/^Bagian\s+(?:Kesatu|Kedua|Ketiga|Keempat|Kelima|Keenam|Ketujuh|Kedelapan|Kesembilan|Kesepuluh)/i.test(line)) return 'BAGIAN';
  ...
  level(type) {
    switch (type) {
      case 'BAB': return 1;
      case 'BAGIAN': return 2;
      case 'PARAGRAF': return 3;
      case 'PASAL': return 4;
      case 'AYAT_HEADER': return 5;
      case 'AYAT': return 5;
      case 'HURUF': return 6;
      case 'NOMOR': return 6;
  ...
  serialize(blocks) {
    return blocks.map(b => this.serializeBlock(b, 0)).join('\n').replace(/\n{3,}/g, '\n\n') + '\n';
  }
  ...
  serializeBlock(block, depth) {
    ...
    switch (block.type) {
      ...
      case 'NOMOR':
      case 'BULLET':
      case 'BODY': out = indent + block.text + '\n'; break;
      default: out = block.text + '\n';
    }
