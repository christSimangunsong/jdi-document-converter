...
- **Progress detail (v30.5)**: SSE `progress` = `{pct, phase, fileName, page, totalPages, detail}`; `detail` = aktivitas berjalan (Table-OCR halaman i/N + engine + tabel ditemukan) → tampil di `#progressDetail`; pct JDIH ×100 (bar naik natural 0-100); gotcha: `engine.js performOcrBlocks` wajib meneruskan `options` ke router
