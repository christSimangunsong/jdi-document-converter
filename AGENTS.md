...
- **Progress UI detail (v30.5)**: event SSE `progress` = `{pct, phase, fileName, page, totalPages, detail}` — `detail` = pesan aktivitas berjalan (table-OCR per halaman: engine, halaman i/N, tabel ditemukan; juga "Table-OCR dimulai/selesai"); UI menampilkan di `#progressDetail` (baris kecil, hilang saat tidak ada detail). `processBuffer`'s `progress()` melacak `lastPct`/`lastPhase` — event detail (`progress(null, null, {detail})`) mengirim pct/phase terakhir tanpa reset. **pct JDIH ×100** (0-1 pipeline → 0-100 UI; sebelumnya bar macet di 1% dan lompat saat done). Fase JDIH tambahan: `[JDIH] Mengunduh PDF` (2%) / `[JDIH] Memeriksa file` (4%). **Gotcha**: `src/ocr/engine.js` `performOcrBlocks(imageBuffers, onProgress, options)` WAJIB meneruskan `options` ke `ocrRouter` — kalau tidak, `transcription` & `onDetail` dari server.js hilang (transcription selamat hanya karena fallback config). 234 unit tests.
